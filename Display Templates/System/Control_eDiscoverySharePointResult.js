...
{
	if(!ctx.ClientControl.get_showPaging() && totalRows > resultsPerPage)
	{
ms_outHtml.push(''
,'        <span class="ms-srch-resultscount" id="ResultCount">'
,'            ', String.format($htmlEncode(Srch.Res.rs_EdiscoveryTopResultCount), $htmlEncode(totalRows.localeFormat("N0")), $htmlEncode(resultsPerPage)) ,''
,'        </span>'
); 
	}
	else
	{
ms_outHtml.push(''
,'        <span class="ms-srch-resultscount" id="Span1">'
...
,'        <div id="NoResult" class="ms-srch-result-noResults">'
         ...
         if (ctx.ClientControl.get_searchSkipped())
         {
ms_outHtml.push(''
,'            <div class="ms-textLarge ms-srch-result-noResultsTitle">', $htmlEncode($resource("rs_NoResultsMessageWhenSearchSkipped")) ,'</div>'
); 
        }
        else
        {
...
,'                            <img src="', $urlHtmlEncodeString(imagesUrl) ,'" class="', $htmlEncode(prevPageIconClass) ,'" alt="', $htmlEncode(firstPage.title) ,'" />'
,'                        </span>'
,'                    </a>'
);
        }
...
,'                        <span class="ms-promlink-button-image">'
,'                            <img src="', $urlHtmlEncodeString(imagesUrl) ,'" class="', $htmlEncode(nextPageIconClass) ,'" alt="', $htmlEncode(lastPage.title) ,'" />'
,'                        </span>'
,'                    </a>'
);
        }
