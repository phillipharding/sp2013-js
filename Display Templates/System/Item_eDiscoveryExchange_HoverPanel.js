...
path.overrideValueRenderer($urlHtmlEncodeValueObject);
...
,'                            <img src="', $urlHtmlEncodeString(SP.Utilities.VersionUtility.getImageUrl(Srch.SU.closeImage)) ,'" alt="', $htmlEncode($resource('hp_Tooltip_Close')) ,'" />'
,'                        </a>'
,'                    </div>'
);
if (!path.isEmpty) 
...
,'                        <a clicktype="Result" id="', $htmlEncode(id + HP.ids.title) ,'" title="', subject ,'" href="', path ,'">'
    ...
    {
ms_outHtml.push(''
,'                            <img class="ms-srch-item-icon ms-positionRelative" alt="', $htmlEncode($resource('hp_EDisc_AttachmentsTooltip')) ,'" id="', $htmlEncode(id + Srch.U.Ids.icon) ,'" onload="this.style.display=\'inline\';this.style.bottom=\'-5px\';" src="', $urlHtmlEncodeString(SP.Utilities.VersionUtility.getImageUrl('attach16.png')) ,'" onerror="this.style.display=\'none\';" />'
);
    }
ms_outHtml.push(''
