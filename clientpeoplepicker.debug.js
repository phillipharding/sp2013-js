function $_global_clientpeoplepicker() {
    ...
    SPClientPeoplePicker.prototype = {
        ...
        UpdateUnresolvedUser: function(results, user) {
            ...
            if (Boolean(entity.IsResolved)) {
                ...
                this.AddResolvedUserToLocalCache(SPClientPeoplePicker.AugmentEntity(entity), user.ResolveText);
            }
            else {
        ...
        AddResolvedUserToLocalCache: function(resolvedEntity, resolveText) {
            if (resolvedEntity != null && resolveText != null && resolveText != '') {
                if (this.ShouldUsePPMRU() && SPClientPeoplePicker.IsUserEntity(resolvedEntity))
                    this.PPMRU.SetItem(resolveText, resolvedEntity);
            }
        }
    };
    SPClientPeoplePicker.TestLocalMatch = function(strSearchLower, dataEntity) {
        ...
        if (dataEntity.DomainText != uri.getAuthority())
            return false;
        if (typeof dataEntity.LocalSearchTerm != 'undefined') {
            var strDataItemTerm = dataEntity.LocalSearchTerm;

            if (strDataItemTerm != null && strDataItemTerm.indexOf(strSearchLower) == 0)
                return true;
        }
        var userNameKey = dataEntity.Key;
        var divideIdx = userNameKey.indexOf('\\');

        if (divideIdx != -1 && divideIdx != userNameKey.length - 1)
            userNameKey = userNameKey.substr(divideIdx + 1);
        ...
        if ((userNameKey.toLowerCase()).indexOf(strSearchLower) == 0 || (dataEntity.DisplayText.toLowerCase()).indexOf(strSearchLower) == 0 || (emailKey.toLowerCase()).indexOf(strSearchLower) == 0)
            return true;
        return false;
    };
    ...
    SPClientPeoplePicker.AugmentEntitySuggestions = function(pickerObj, allEntities, mergeLocal) {
        ...
        for (var entityIdx = 0; entityIdx < entityCount; entityIdx++) {
            ...
            if (mergeLocal && pickerObj.CurrentLocalSuggestionsDict[key.toLowerCase()] != null)
                continue;
            resArray.push(SPClientPeoplePicker.AugmentEntity(entity));
        }
        return resArray;
    };
    SPClientPeoplePicker.AugmentEntity = function(entity) {
        ...
        if (entity[SPClientPeoplePicker.DescriptionName] != null)
            descriptionText = entity[SPClientPeoplePicker.DescriptionName];
        var hasEmail = entity.EntityData != null && entity.EntityData.Email != null;
        var email = hasEmail ? entity.EntityData.Email : '';
        var claimProvider = entity.ProviderDisplayName != null ? entity.ProviderDisplayName : '';

        titleText = email != '' && claimProvider != '' ? email + '\n' + claimProvider : email + claimProvider;
        if (Boolean(descriptionText))
            titleText += '\n' + descriptionText;
        ...
        entity[SPClientPeoplePicker.TitleTextProperty] = titleText;
        var uri = new URI(document.URL);

        entity[SPClientPeoplePicker.DomainProperty] = uri.getAuthority();
        return entity;
    };
    ...
    SPClientPeoplePickerProcessedUser.prototype = {
        ...
        UpdateResolvedUser: function(newUserInfo, strNewElementId) {
            ...
            var deleteUserElement = document.getElementById(this.DeleteUserElementId);
            ...
            deleteUserElement.title = StBuildParam(Strings.STS.L_SPClientDeleteProcessedUserAltText, this.DisplayName);
            ...
            this.DeleteUserElementId = (deleteUserElement.id = strNewElementId + '_DeleteUserLink');
        ...
        ULSa9l:
            ...
            if (!this.ResolvedUser)
                personSpan.push('</a>');
            personSpan.push('</span>');
            var deleteAltText = StBuildParam(Strings.STS.L_SPClientDeleteProcessedUserAltText, this.DisplayName);

            personSpan.push('<a class="sp-peoplepicker-delImage" title="');
            personSpan.push(STSHtmlEncode(deleteAltText));
            personSpan.push('" id="');
            personSpan.push(STSHtmlEncode(this.DeleteUserElementId));
...
function SPClientPeoplePicker_CallbackOnAutoFillClose(elmInputId, userObject) {
    ...
    if (userObject != null) {
        ...
        pickerObj.AddResolvedUserToLocalCache(userObject, strCacheKey);
    }
...
function SPClientPeoplePicker_OnEditorFocus(e) {
    ...
    if (!objPicker.QueryScriptInit) {
        ...
        ULSa9l:
            ;
            objPicker.QueryScriptInit = true;
        });
    }
    if (!objPicker.EditorWidthSet)
        objPicker.EnsureEditorWidth();
    var topLevelElm = document.getElementById(objPicker.TopLevelElementId);
...
function SPClientPeoplePicker_OnEditorKeyDown(e) {
    ...
    if (e.ctrlKey && keynum == 75) {
        ...
        var backTab = e.shiftKey;

        if (backTab || !pickerObj.IsAutoFillOpen()) {
            pickerObj.CloseAutoFill();
        ...
        elmInput.size = Math.max(elmInput.value.length + 1, 1);
    }
    if (cancelKeyPress)
        CancelEvent(e);
    return !cancelKeyPress;
...
function SPClientPeoplePicker_OnEditorKeyPress(e) {
    if (e == null)
        e = window.event;
    var keynum = GetEventKeyCode(e);
    var elmInput = GetEventSrcElement(e);
...
function SPClientPeoplePickerProcessedUser(userInfo, elmProcessedUserId, fResolved) {
    ...
    this.DeleteUserElementId = elmProcessedUserId + '_DeleteUserLink';
    this.PresenceElementId = elmProcessedUserId + '_PresenceContainer';
    this.UserInfo = userInfo;
    this.ResolvedUser = fResolved;
    this.ResolveText = userInfo.ResolveText;
